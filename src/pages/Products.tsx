...
export default function Products() {
  ...
  useEffect(() => {
    ...
    setfilteredProducts(
      products
        ?.filter((product) => product.name.toLowerCase().includes(val))
  ...
  return (
    <HomeLayout>
      <>
        <div className="filters mt-4 mb-5 px-2 bg-white self-center flex items-center sticky top-0 gap-x-4 z-20 w-full md:w-1/2">
          <SearchBar placeholder="Search" search={search?.split("=")[1]} />
          <LuSettings2 className="filter-icon h-12 w-10 bg-light text-secondary flex-none text-4xl bg-slate-200 p-1 font-normal rounded" />
        </div>
