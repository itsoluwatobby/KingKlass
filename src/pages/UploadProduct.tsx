...
import TextArea from "../components/appComponents/TextArea";
import InputText from "../components/appComponents/InputText";
import { LiaTimesSolid } from "react-icons/lia";
import { hoverClass } from "../utility/constants";


const initProduct = { productName: '', description: '', price: '', duration: '', productImages: [] }
        ...
        <section className="flex flex-col lg:w-1/2 w-full gap-y-7">
          ...
          <div className="flex text-sm items-center w-full p-1 bg-gray-200 rounded-[3px]">
            <button
              ...
              className={`p-1 rounded-[3px] cursor-pointer w-1/2 ${active === 'uploaded' ? 'text-[#8B4513] font-bold bg-white' : 'bg-gray-200'} hover:opacity-95 transition-all`}>Uploaded</button>
            <button
              onClick={() => setActive('draft')}
              className={`w-1/2 p-1 rounded-[3px] cursor-pointer ${active === 'draft' ? 'text-[#8B4513] font-bold bg-white' : 'bg-gray-200'} hover:opacity-95 transition-all`}>Draft</button>
          </div>
          ...
          <div className="border rounded-md bg-gray-100 p-1.5 flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="flex bg-white items-center flex-wrap md:w-[75%] p-1 gap-x-2 w-[90%]">
                {
                  categories.map(cat => (
                    <button key={cat} className={"rounded-sm focus:outline-0 border-0 w-fit p-0.5 px-1 bg-gray-200 flex items-center gap-x-1.5" + hoverClass}>{cat} <LiaTimesSolid className='text-sm font-bold text-black' />
                    </button>
                  ))
                }
              </div>
            ...
            <div className="mt-2 flex items-center justify-between">
              <div className="flex items-center gap-3 relative">
                ...
                <InputText type="tel"
                  ...
                  id="price" setInputText={setNewProduct}
                  placeholder="" classNames="rounded-[5px] h-9"
                />
                <span className="text-gray-500 absolute left-12 font-semibold">&#x20A6;</span>
              </div>
              <div className="flex items-center gap-3 relative">
                <span className="text-[#6E6E6E]">Duration</span>
                <InputText type="tel"
                  value={duration} name="duration"
                  id="duration" setInputText={setNewProduct}
                  placeholder="" classNames="rounded-[5px] h-9"
                />
            ...
            <div className="flex items-center gap-4">
              {
                [1,2,3,4,5].map((i) => (
                  <figure key={i} className="bg-gray-300 relative w-14 h-12 rounded-[3px]">
                    <img src="" alt="" className="w-full h-full object-cover rounded-[3px]" />
    ...
    <div className="hover:opacity-80 transition-opacity font-sans flex items-center p-2 border-[1px] even:bg-slate-50 odd:bg-slate-100 justify-between text-[12px] w-full">
      <Link to={``} className="flex-none w-[70%] flex items-center">
        <figure className="w-10 h-10 rounded-[3px] flex-none">
          <img src={productObj.image} alt={productObj.productName} className="object-cover h-full w-full rounded-[3px]" />
        </figure>
        <div className="flex items-center flex-col gap-y-1 px-3">
          <span>{productObj.productName}</span>
          <span className="font-sans font-semibold text-[12px]">&#x20A6;{formatPrice(productObj.price)}</span>
        </div>
      </Link>
