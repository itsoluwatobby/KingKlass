...
export const TrackCards = ({ title, Icon, count, updatedAt }: TrackCardsProps) => {
  ...
  return (
    <article className="cursor-default rounded-md border flex md:items-center md:gap-x-5 md:w-fit md:flex-row flex-col p-1.5 md:px-6 md:py-4 w-full gap-y-2.5 shadow-sm">
      ...
      <h4 className="hidden midscreen:block font-sans text-2xl font-semibold text-[#8B4513]">{typeof count === 'number' ? checkCount(count) : <span>&#x20A6;{formatPrice(count)}</span>}</h4>

      <span className="hidden midscreen:block text-[13px] font-sans text-[#6E6E6E]">Last updated {format(updatedAt)}</span>
      ...
      <div className="hidden md:flex flex-col">
        ...
        <span className="text-xs whitespace-nowrap font-sans text-[#6E6E6E]">Last updated {format(updatedAt)}</span>
      </div>
